 ...
 * 获取所有商品
 */
export async function getGoods(category?: string): Promise<ApiResponse<Goods[]>> {
  const query = category ? `?category=${encodeURIComponent(category)}` : ''
  return apiRequest<Goods[]>(`/goods${query}`)
}

/**
 * 获取特定商品详情
 */
export async function getGoodsById(id: number): Promise<ApiResponse<Goods>> {
  return apiRequest<Goods>(`/goods/${id}`)
}
